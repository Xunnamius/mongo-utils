...
import {
  ...
  name as packageName
} from 'rootverse+mongo-test:package.json';
...
const debug = createDebugLogger({ namespace: 'mongo-test' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+mongo-test:package.json'))),
    packageName,
    packageExports
  );
});
