...
import {
  ...
  name as packageName
} from 'rootverse+mongo-item:package.json';
...
const debug = createDebugLogger({ namespace: 'mongo-item' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+mongo-item:package.json'))),
    packageName,
    packageExports
  );
});
