...
import {
  ...
  name as packageName
} from 'rootverse+mongo-object-id-pseudo-sort:package.json';
...
const debug = createDebugLogger({ namespace: 'mongo-object-id-pseudo-sort' }).extend(
  TEST_IDENTIFIER
);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(
      toAbsolutePath(
        require.resolve('rootverse+mongo-object-id-pseudo-sort:package.json')
      )
    ),
    packageName,
    packageExports
